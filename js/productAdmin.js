...
document.getElementById('formSubmit').addEventListener('submit', function (e){
  ...
  for(let i = 0; i < products.length; i++){
    if(inputProductName== products[i].name ){
    ...
    if(inputProductId== products[i].id){
    ...
    if(inputProductImg == products[i].image){
      ...
      for(let i = 0; i < products.length; i++){
          ...
          if(products[i].id == flag){
            console.log("222222222222222222",products[i]);
              ...
              // document.getElementById("addAndEditProduct").innerText = "Lưu lại";
              flag = -1;
              renderProduct();
              clearForm();
              return;
